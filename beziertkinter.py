...
def start_drag(event):
    global drag_point
    drag_point = event.widget

def on_drag(event, rect):
    x, y = get_center(canvas.coords(rect))
    for l in lines:
        canvas.delete(l)
    canvas.move(rect, event.x - x, event.y - y)
...
def place_points(event):
    global start
    if len(start) <= 4:
        rect = canvas.create_rectangle(event.x - 3, event.y - 3, event.x + 3, event.y + 3, fill="black")
        start.append(rect)
        if (len(start) == 4):
            canvas.unbind("<Button-1>")
            for p in start:
                canvas.tag_bind(p, "<B1-Motion>", lambda event, r=rect: on_drag(event, r))
            store_bezier_in_array()
            draw_bezier()
